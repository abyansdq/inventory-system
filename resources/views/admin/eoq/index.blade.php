    ...
    <div class="card bg-blue-50 border-blue-200 py-4">
        <div class="flex gap-3">
            ...
            <div class="text-sm text-blue-800">
                ...
                <p class="mt-1">
                    Formula: <code class="bg-blue-100 px-1 rounded">EOQ = √(2DS/H)</code>
                    — D = Demand Tahunan, S = Biaya Pesan, H = Biaya Simpan/unit/tahun
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="card py-4">
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            @if(request()->hasAny(['search','category_id']))
                <a href="{{ route('admin.eoq.index') }}" class="btn-secondary">Reset</a>
            @endif
        </form>
    </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">EOQ Terakhir</th>
                    ...
                    @forelse($items as $i => $item)
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="font-semibold {{ $item->status_stok === 'menipis' ? 'text-yellow-600' : ($item->status_stok === 'habis' ? 'text-red-600' : 'text-green-600') }}">
                            ...
                            <td class="text-center">
                                @if($latestEoq)
                                    ...
                                    <span class="text-xs text-gray-400">{{ $item->satuan }}</span>
                                @else
                                    <span class="text-gray-300 text-xs">Belum dihitung</span>
                                @endif
                            </td>
                            <td class="text-center">
                                @if($latestEoq)
                            ...
                            <td class="text-sm text-gray-500">
                                {{ $latestEoq ? $latestEoq->tanggal_hitung->format('d/m/Y H:i') : '—' }}
                            </td>
                            <td class="text-center">
                                <a href="{{ route('admin.eoq.show', $item) }}"
