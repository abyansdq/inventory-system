    ...
    <div class="card py-4">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            ...
            @if(request()->hasAny(['status','tanggal_dari','tanggal_sampai']))
                <a href="{{ route('manajer.reports.procurement') }}"
                   class="btn-secondary">Reset</a>
            @endif
            <div class="ml-auto flex gap-2">
                <a href="{{ route('manajer.reports.export.pdf', 'procurement') }}?{{ http_build_query(request()->all()) }}"
                   class="btn-danger text-sm" target="_blank">Export PDF</a>
                <a href="{{ route('manajer.reports.export.excel', 'procurement') }}?{{ http_build_query(request()->all()) }}"
